...
class WP_Twitter_profile_widget extends WP_Widget
{
	...
	function __construct()
	{
		...
		$this->add_field('twitter_count', 'Number of tweets to show (enter \'0\' to only show your profile)', '0', 'number');
		$this->add_field('twitter_show_bio', 'Show twitter bio', 'on', 'checkbox');
		$this->add_field('twitter_show_counts', 'Show twitter stats (tweets, following, followers)', 'on', 'checkbox');
		$this->add_field('twitter_show_follow', 'Show follow button', 'on', 'checkbox');
		$this->add_field('twitter_target', 'Open links in a new tab', 'off', 'checkbox');
		...
		$this->add_field('twitter_timeout', 'Cache timout in minutes', '0', 'number' );
		...
		foreach($this->fields as $field_name => $field_data)
		{
			...
			?>
				<p>
					...
					<?php
					if (esc_attr($instance[$field_name]) == 'on')
					{
						echo ' checked';
					}
					?>
